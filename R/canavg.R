canavg <- function(gla_detailed_report_txt, eye_y=-8, eye_z=10, display_diagnostics=TRUE) {
  ...
  n_rand_point <- 2500            # number of random points to be generated to visualize half-sphere surface
    ...
    for (i in 1:nrow(out)) {
      if (i == 1) {
        ...
        out2 <- out %>%
          mutate(azmAng = NA) %>%
          mutate(altAng = NA) %>%
          mutate(index = as.factor(cardDirs[j])) %>%
  ...
  raw_data <- readLines(gla_detailed_report_txt)
  ...
  gla_report <- read.delim(gla_detailed_report_txt, sep = ";", header = FALSE, skip = 2) %>%
    # convert to tibble
    as_tibble() %>%
  ...
  for (p in 1:4) {
    if (p == 1) {
      dirTotals <- rep(NA,4)
      names(dirTotals) <- cardDirs
    }
    dirTotals[p] <- out3 %>%
      filter(index == cardDirs[p]) %>%
